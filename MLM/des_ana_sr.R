...
sr_combined_table <- sr_combined_long %>%
  tbl_strata(
    strata = "Outcome",  # 按结局分层
    .tbl_fun =
      ~ .x %>%
      tbl_summary(
        by = Status,  # 按状态分组
        include = c(urban_rural, education, pche_quartile, gender, marital, age_group),
        statistic = list(all_categorical() ~ "{n} ({p}%)"),
        digits = all_categorical() ~ c(0, 1),
        missing = "no",
        percent = "row"  # 关键修改：计算行百分比
      ) %>%
      modify_header(stat_by = "**{level}**") %>%
      modify_spanning_header(all_stat_cols() ~ "**Self-reported Status**")
  ) %>%
  modify_header(label ~ "**Predictor**") %>%
  bold_labels() %>%
  modify_caption("Characteristics by Health Outcomes (Row Percentages)")
