...
# 2. 提取三个模型的系数
hypertension <- get_coefs(model.hypertension_pre_2015)
diabetes <- get_coefs(model.diabetes_pre_2015)
dyslipidemia <- get_coefs(model.dyslipidemia_pre_2015)
...
final_table <- formatted_table %>%
  kable(
    format = "html",
    caption = "<b>表1. 城乡变量效应in prevalence跨模型比较结果</b>",
  ...
  footnote(
    general = "注：表格显示了prevalence模型中城乡变量的系数差异比较结果。",
    general_title = "",
...
save_kable(final_table, file = "prevalence wald test.docx")
